      ...
      <div className='w-full flex justify-between items-center sm:p-4 p-2 md:min-h-[100px] h-fit text-white bg-[var(--primary)]'>
        ...
        <div className='flex items-center justify-between gap-5 max-md:hidden'>
          <input
            ...
            className='py-0.5 px-4 hover:outline text-sm bg-white text-[var(--primary)] font-medium outline-blue-600 rounded border-gray-600 border'
          />
          <NavLink to={'/login'} className='font-medium text-xs hover:underline'>
            Login
          </NavLink>
          <NavLink to={'/register'} className='font-medium text-xs hover:underline'>
            Register
          </NavLink>
        </div>
        {searchActive ? (
          ...
          <div className='flex text-xs font-bold text-gray-800 items-center gap-4 md:hidden'>
            <Search strokeWidth={3} onClick={() => setSearchActive(true)} className='text-white' />
          ...
          {sports.map((item, index) => (
            <li key={item.typeId} className='flex items-center'>
              ...
              <NavLink
                ...
                className='font-semibold uppercase text-nowrap text-xs'>
                {item.name}
              </NavLink>
            </li>
          ))}
